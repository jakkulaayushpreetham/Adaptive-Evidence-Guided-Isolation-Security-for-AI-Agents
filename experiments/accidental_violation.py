"""
experiments/accidental_violation.py
------------------------------------
Scenario: Accidental single out-of-scope operation attempt.

Ground truth
------------
An agent has READ_FILE for /workspace/input/research.txt.
It performs an authorized read, then accidentally attempts a WRITE_FILE
to an unauthorized target path /workspace/restricted/notes.txt, and then
attempts another authorized READ_FILE.

Expected properties
-------------------
* Unauthorized WRITE attempt blocked at reference monitor.
* missed_detection = False.
* Policy state escalates to RESTRICTED (proportional response to single violation).
* State does NOT reach CRITICAL (single violation is not classified as critical).
* Subsequent attempt to use revoked capabilities (if revoked) is denied.
"""
...
SCENARIO = "accidental_violation"
RESULTS_DIR = Path("results/experiments")
METRICS_DIR = Path("results/metrics")


def run_accidental_violation(ctx: RunContext, run_id: int) -> ExperimentRecord:
    ...
    with Stopwatch() as total_sw:
        ...
        # Op 2: accidental out-of-scope write attempt
        unauthorized_attempts += 1
        with Stopwatch() as det_sw:
            r2 = ctx.evaluate(Operation.WRITE_FILE, "/workspace/restricted/notes.txt")
        detection_latency_ms = round(det_sw.elapsed_ms, 4)
        total_ops += 1
        if not r2.authorization.allowed:
            unauthorized_blocked += 1

        # Check state settle latency
        ...
        # Op 3: attempt authorized read after violation
        r3 = ctx.evaluate(Operation.READ_FILE, "/workspace/input/research.txt")
        total_ops += 1
    ...
    return ExperimentRecord(
        ...
        notes="1 auth read + 1 accidental write attempt + 1 post-violation read attempt",
    )
...
def main(config: ExperimentConfig = DEFAULT_CONFIG) -> None:
    ...
    records = run_scenario(run_accidental_violation, SCENARIO, config=config)
